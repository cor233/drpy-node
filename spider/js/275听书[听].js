...
var rule = {
    ...
    二级: async function () {
        ...
        let title = pdfh(html, 'h1.text-2xl&&Text') || '';
        ...
        let desc = pdfh(html, '.bg-white.p-4 p.text-gray-600&&Text') || '';
        let tabs = [], lists = [];
        let chapterItems = pdfa(html, '.grid.grid-cols-1 a[id^="chapter-pos-"]');
    ...
    lazy: async function () {
        ...
        // 方法1：提取 APlayer 初始化中的 audio url
        // 匹配 url: '...' 这样的字符串，捕获引号内的内容
        let match = html.match(/url:\s*'([^']+)'/);
        if (match) {
            let audioUrl = match[1];
            // 可能需要进行 URL 拼接（相对路径极少，但保留）
